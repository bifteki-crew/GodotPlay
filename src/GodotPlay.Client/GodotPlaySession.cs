...
public class GodotPlaySession : IGodotPlaySession
{
    ...
    public async Task<ActionResult> MouseMoveAsync(float x, float y, float speed = 0, CancellationToken ct = default)
        => await _client.MouseMoveAsync(new MouseMoveRequest { X = x, Y = y, Speed = speed }, cancellationToken: ct);

    public async Task<ActionResult> MouseClickAsync(float x, float y, int button = 1, int clickCount = 1, CancellationToken ct = default)
        => await _client.MouseClickAtAsync(new MouseClickRequest { X = x, Y = y, Button = button, ClickCount = clickCount }, cancellationToken: ct);

    public async Task<ActionResult> MouseDownAsync(float x, float y, int button = 1, CancellationToken ct = default)
        => await _client.MouseButtonEventAsync(new MouseButtonRequest { X = x, Y = y, Button = button, Pressed = true }, cancellationToken: ct);

    public async Task<ActionResult> MouseUpAsync(float x, float y, int button = 1, CancellationToken ct = default)
        => await _client.MouseButtonEventAsync(new MouseButtonRequest { X = x, Y = y, Button = button, Pressed = false }, cancellationToken: ct);
    ...
    public async Task<ActionResult> KeyUpAsync(string keyLabel, CancellationToken ct = default)
        => await _client.KeyUpAsync(new KeyRequest { KeyLabel = keyLabel, Pressed = false }, cancellationToken: ct);
    ...
    public async Task<ActionResult> TouchDragAsync(float fromX, float fromY, float toX, float toY, int steps = 10, int durationMs = 300, CancellationToken ct = default)
        => await _client.TouchDragAsync(new TouchDragRequest { FromX = fromX, FromY = fromY, ToX = toX, ToY = toY, Steps = steps, DurationMs = durationMs }, cancellationToken: ct);
    ...
    public async Task<ActionResult> ActionPressAsync(string action, float strength = 1, int durationMs = 100, CancellationToken ct = default)
        => await _client.ActionPressAsync(new ActionPressRequest { Action = action, Strength = strength, DurationMs = durationMs }, cancellationToken: ct);
    ...
    public async Task<ActionResult> DragToAsync(string fromNodePath, string toNodePath, int steps = 10, int durationMs = 300, CancellationToken ct = default)
        => await _client.DragToAsync(new DragRequest { FromNodePath = fromNodePath, ToNodePath = toNodePath, Steps = steps, DurationMs = durationMs }, cancellationToken: ct);
