...
public interface IGodotPlaySession : IAsyncDisposable
{
    ...
    // Low-Level Input
    Task<ActionResult> MouseMoveAsync(float x, float y, float speed = 0, CancellationToken ct = default);
    Task<ActionResult> MouseClickAsync(float x, float y, int button = 1, int clickCount = 1, CancellationToken ct = default);
    Task<ActionResult> MouseDownAsync(float x, float y, int button = 1, CancellationToken ct = default);
    Task<ActionResult> MouseUpAsync(float x, float y, int button = 1, CancellationToken ct = default);
    ...
    Task<ActionResult> KeyUpAsync(string keyLabel, CancellationToken ct = default);
    Task<ActionResult> TouchAsync(int index, float x, float y, bool pressed, CancellationToken ct = default);
    Task<ActionResult> TouchDragAsync(float fromX, float fromY, float toX, float toY, int steps = 10, int durationMs = 300, CancellationToken ct = default);
    ...
    Task<ActionResult> ActionPressAsync(string action, float strength = 1, int durationMs = 100, CancellationToken ct = default);
    ...
    Task<ActionResult> DragToAsync(string fromNodePath, string toNodePath, int steps = 10, int durationMs = 300, CancellationToken ct = default);
